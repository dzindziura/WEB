...
mongo.connect((error, client) => {
  ...
  exp.locals.ports      = client.db(db_name).collection("ports");
  exp.locals.piers        = client.db(db_name).collection("piers");
  exp.locals.ships       = client.db(db_name).collection("ships");
  exp.locals.cured_ships = client.db(db_name).collection("cured_ships");
...
// ... для запиту /get_ports
exp.get("/get_ports", (req, res) => {

  collection = req.app.locals.ports;
...
// ... для запиту /get_piers
exp.get("/get_piers", (req, res) => {

  collection = req.app.locals.piers;
  collection.find({}).toArray((error, result) => {
...
// ... для запиту /get_ships
exp.get("/get_ships", (req, res) => {

  collection = req.app.locals.ships;
  collection.find({}).toArray((error, result) => {
...
// ... для запиту /get_cured_ships
exp.get("/get_cured_ships", (req, res) => {

  collection = req.app.locals.cured_ships;
  collection.find({}).toArray((error, result) => {
...
exp.get("/get_last_port_id", (req, res) => {

  collection = req.app.locals.identificators;
  collection.find({name: "last_port_id"}).toArray((error, result) => {
...
// ... для запиту /get_last_pier_id
exp.get("/get_last_pier_id", (req, res) => {

  collection = req.app.locals.identificators;
  collection.find({name: "last_pier_id"}).toArray((error, result) => {
...
// ... для запиту /get_last_ship_id
exp.get("/get_last_ship_id", (req, res) => {

  collection = req.app.locals.identificators;
  collection.find({name: "last_ship_id"}).toArray((error, result) => {
...
exp.put(["/set_ports",
  ...
  switch (collection) {
    case 1: collection = req.app.locals.ports;      break;
    case 2: collection = req.app.locals.piers;        break;
    case 3: collection = req.app.locals.ships;       break;
    case 4: collection = req.app.locals.cured_ships; break;
